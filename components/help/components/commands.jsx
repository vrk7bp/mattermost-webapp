...
export default function HelpCommands() {
    ...
    message.push(localizeMessage('help.commands.intro', 'Slash commands perform operations in Scrybe by typing into the text input box. Enter a `/` followed by a command and some arguments to perform actions.\n\nBuilt-in slash commands come with all Scrybe installations and custom slash commands are configurable to interact with external applications. Learn about configuring custom slash commands on the [developer slash command documentation page](http://docs.mattermost.com/developer/slash-commands.html).'));
    message.push(localizeMessage('help.commands.builtin', '## Built-in Commands\nThe following slash commands are available on all Scrybe installations:'));
